    ...
    <tbody>
      {!isLoading &&
        paginatedData.map((item, index) => (
          <tr
            key={index}
            className={`${index % 2 === 0 ? "bg-gray-50 dark:bg-gray-200" : "bg-white dark:bg-gray-300"}`}
          >
            {headers.map((header) => (
              <td key={header.column} className="px-4 py-2 text-gray-700">
                {header.column === "abonne" ? (
                  item[header.column] ? (
  ...
  return (
    <div className="flex justify-between items-center mt-4 dark:bg-darkColor dark:text-white">
      ...
      <ul className="flex space-x-2">
        <li>
          <button
            className={`px-3 py-1 border border-black dark:border-white rounded-lg ${currentPage === 1 ? "bg-gray-200 cursor-not-allowed" : ""}`}
        ...
        {renderPageNumbers().map((pageNumber, index) => (
          <li key={index}>
            <button
              className={`px-3 py-1 border border-black dark:border-white rounded-lg ${currentPage === pageNumber ? "bg-blue-500 text-white" : ""}`}
        ...
        <li>
          <button
            className={`px-3 py-1 border border-black dark:border-white rounded-lg ${currentPage === totalNumberOfPages ? "bg-gray-200 cursor-not-allowed" : ""}`}
  ...
  return (
    <div className="min-h-screen p-4 bg-white shadow-md rounded-lg dark:bg-darkColor">
      <div className="flex justify-between mb-4">
        <div>
          <label className="hidden lg:inline md:inline text-gray-600 rounded-lg dark:text-white">Show</label>
          <select
