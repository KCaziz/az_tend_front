...
const TableHeader = ({
        ...
        {!isLoading &&
          paginatedData.map((item, index) => (
            <tr
              key={index}
              className={`${
                index % 2 === 0 ? "bg-gray-50 dark:bg-gray-200" : "bg-white dark:bg-gray-300"
              }`}
            >
        {headers.map((header) => (
          <td key={header.column} className="px-4 py-2 text-gray-700">
            {/* Si c'est un tableau, on affiche les éléments */}
            {Array.isArray(item[header.column]) 
              ? item[header.column].map((sector) => sector.title).join(", ")  // Par exemple, pour les titres des secteurs
              : item[header.column]}
          </td>
        ))}
              {/* Colonne des actions */}
              <td className="px-4 py-2 text-gray-700">
                <Button color="primary" onClick={() => onEdit(item._id)} className="my-1">
                  <FaPencilAlt />
                </Button>
  ...
  // Tu peux garder le reste du code inchangé pour la pagination et autres fonctionnalités
...
const Table = ({ headers, data, isLoading, loadingTag, edit_func, delete_func }) => {
  ...
  const [sortColumn, setSortColumn] = useState(headers[0].column); // Colonne triée
  const [sortDirection, setSortDirection] = useState("asc"); // Direction du tri
  const [selectedType, setSelectedType] = useState(null); // Filtre des types
  ...
  return (
    <div className="min-h-screen p-4 bg-white shadow-md rounded-lg dark:bg-darkColor">
      ...
      <div className="flex justify-between mb-4">
        <div>
          <label className="text-gray-600 dark:text-white"> <span className="hidden md:inline">Show</span></label>
          <select
      ...
      {/* Filtre de types */}
      <div className="flex items-center justify-center overflow-x-auto mb-4">
